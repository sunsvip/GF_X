...
//#endif
    ...
    public void ShowInterstitialAd(ADTriggerPoint adPoint, GameFrameworkAction<ADResult> callback = null)
    {
        ...
        this.SendEvent("Ads", new Dictionary<string, string> { ["Interstitial"] = Utility.Text.Format("Trigger_{0}", adPoint.ToString()) });
    ...
    private void OnInterstitialAdOpen(bool openSuccess)
    {
        ...
        if (openSuccess)
        {
            EnqueueAdEvent(delegate
            {
                ...
                this.SendEvent("Ads", new Dictionary<string, string> { ["Interstitial"] = Utility.Text.Format("Play_{0}", triggerPoint.ToString()) });
            });
        }
    }
    ...
    private void OnInterstitialAdClosed(bool obj)
    {
        ...
        {
            this.showInterAdCallback?.Invoke(ADResult.Close);
            SendEvent("Ads", new Dictionary<string, string> { ["Interstitial"] = Utility.Text.Format("Finish_{0}", triggerPoint.ToString()) });
            LoadInterstitialAd();
        });
    }
    ...
    private void OnRewardedAdClosed(bool rewardSuccess)
    {
        ...
        if (rewardSuccess)
        {
            ...
            {
                this.showVideoAdCallback?.Invoke();
                SendEvent("Ads", new Dictionary<string, string> { ["Videos"] = Utility.Text.Format("Success_{0}", triggerPoint.ToString()) });
            });
        }
        EnqueueAdEvent(delegate
        {
    ...
    private void OnRewardedAdOpen(bool openSuccess)
    {
        ...
        if (openSuccess)
        {
            EnqueueAdEvent(delegate
            {
                SendEvent("Ads", new Dictionary<string, string> { ["Videos"] = Utility.Text.Format("Play_{0}", triggerPoint.ToString()) });
            });
        }
    }
    ...
    internal void TryTriggerInterAd(ADTriggerPoint triggerType)
    {
        ...
        ShowInterstitialAd(triggerType);
    }
}
/// <summary>
/// 广告触发点
