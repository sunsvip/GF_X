...
public static class LocalizationExtension
{
    public static void LoadLanguage(this LocalizationComponent com, string name, string abTestGroup, object userData)
    {
        ...
        if (!string.IsNullOrWhiteSpace(abTestGroup))
        {
            ...
            if (GF.Resource.HasAsset(UtilityBuiltin.AssetsPath.GetLanguagePath(abTestAssetName)) != GameFramework.Resource.HasAssetResult.NotExist)
            {
                assetName = abTestAssetName;
            }
        }
        com.ReadData(UtilityBuiltin.AssetsPath.GetLanguagePath(assetName), userData);
    }
