    ...
    public sealed class DataTableGenerator
    {
        ...
        private static string GenerateDataTableProperties(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                if (dataTypeKeyword == "JsonData" && !string.IsNullOrWhiteSpace(dataComment))
                {
                    dataTypeKeyword = dataComment;
                }
                stringBuilder
                    .AppendLine("        /// <summary>")
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                if (dataTableProcessor.IsSystem(i))
                {
                    ...
                    {
                        //string vTypeStr = dataTableProcessor.GetComment(i);
                        //if (vTypeStr.EndsWith("[][]"))
                        //{
                        //    stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse2DArray<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i), vTypeStr.Replace("[][]", string.Empty)).AppendLine();
                        //}
                        //else if (vTypeStr.EndsWith("[]"))
                        //{
                        //    stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseArray<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i), vTypeStr.Replace("[]", string.Empty)).AppendLine();
                        //}
                        //else
                        //{
                ...
                {
                    if (languageKeyword == "JsonData")
                    {
                        string dataComment = dataTableProcessor.GetComment(i);
                        if (string.IsNullOrWhiteSpace(dataComment))
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseJsonData(columnStrings[index++]);", dataTableProcessor.GetName(i)).AppendLine();
                        }
                        else
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseJsonData<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i), dataComment).AppendLine();
                        }
                    }
                    else
                    {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                if (dataTableProcessor.IsSystem(i))
                {
                    ...
                    //    if (vTypeStr.EndsWith("[][]"))
                    //    {
                    //        stringBuilder.AppendFormat("                    {0} = DataTableExtension.Parse2DArray<{1}>(binaryReader.Read{2}());", dataTableProcessor.GetName(i), vTypeStr.Replace("[][]", string.Empty), dataTableProcessor.GetType(i).Name).AppendLine();
                    //    }
                    //    else if (vTypeStr.EndsWith("[]"))
                    //    {
                    //        stringBuilder.AppendFormat("                    {0} = DataTableExtension.ParseArray<{1}>(binaryReader.Read{2}());", dataTableProcessor.GetName(i), vTypeStr.Replace("[]", string.Empty), dataTableProcessor.GetType(i).Name).AppendLine();
                    //    }
                    //    else
                    //    {
                ...
                else
                {
                    if (languageKeyword == "JsonData")
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(dataComment))
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseJsonData(binaryReader.ReadString());", dataTableProcessor.GetName(i)).AppendLine();
                        }
                        else
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseJsonData<{1}>(binaryReader.ReadString());", dataTableProcessor.GetName(i), dataComment).AppendLine();
                        }
